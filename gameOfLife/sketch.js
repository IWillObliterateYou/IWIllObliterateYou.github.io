...
function applyRules() {
  let newGrid = generateWhiteGrid(GRID_SIZE, GRID_SIZE);

  // look at every tile on the grid
    ...
    for (let x = 0; x < GRID_SIZE; x ++) {
      ...
      for (let localY = -1; localY <= 1; localY ++) {
        for (let localX = -1; localX <= 1; localX ++) {
          // avoid off the edge
          if (x + localX >= 0 && x + localX < GRID_SIZE && y + localY >= 0 && y + localY < GRID_SIZE) {
            numberOfNeighbors += grid[y + localY][x + localX];
          }
        }
      }

      // don't count yourself
      numberOfNeighbors -= grid[y][x];
